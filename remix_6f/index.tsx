...
const MISSION_TYPES = ['🔴', '🟡', '🔵', '🟢', '⚪']; 
const JOKER_TYPES = ['🤡', '😭', '😳', '👽', '🙊'];
const isJoker = (cell: string | null) => cell !== null && JOKER_TYPES.includes(cell);
const JOKER = '🤡'; // Keep as a representative constant for some UI parts
const COLS = 6;
const ROWS = 7;
...
interface JokerShow { r: number; c: number; scale: number; phase: 'GROW' | 'WAIT' | 'FIRE'; emoji: string; }
...
const BallGame: React.FC = () => {
  ...
  const [spawnStats, setSpawnStats] = useState<Record<string, number>>({});
  ...
  const jokerCount = grid.flat().filter(isJoker).length;
  ...
  const triggerExplosion = (r: number, c: number, color: string, isFirework = false, isTreasure = false) => {
    ...
    JOKER_TYPES.forEach(t => colorMap[t] = '#ef4444');
    ...
    // Calculate center of the grid relative to screen
    const gridWidth = 306;
    const gridHeight = 357;
    const startX = (window.innerWidth - gridWidth) / 2;
    const startY = (window.innerHeight - gridHeight) / 2 + 20; // Offset for header

    const x = startX + c * 51 + 25;
    const y = startY + r * 51 + 25;
  ...
  const performJokerFerrisWheel = async (r: number, c: number, emoji: string = JOKER) => {
    const id = `${r}-${c}`;
    setJokerShows(prev => ({ ...prev, [id]: { r, c, scale: 1.0, phase: 'GROW', emoji } }));
    ...
    while (Date.now() - waitStartTime < 1000) {
      triggerExplosion(r, c, emoji, false, true);
    ...
    for (let i = 1; i <= 5; i++) {
      const nr = r + dir[0] * i;
      const nc = c + dir[1] * i;
      triggerExplosion(nr, nc, emoji, true, true);
      playPop(500 + i * 100, 'square', 0.1, 0.1);
      await new Promise(res => setTimeout(res, 70));
    }
  };

  const spawnBouncingBead = (r: number, c: number, emoji: string = JOKER, idOffset: number = 0) => {
    const x = c * 51 + 25;
    const y = r * 51 + 25;
    ...
    for (let i = 0; i < 9; i++) {
      ...
      for (let dr = -2; dr <= 2; dr++) {
        for (let dc = -2; dc <= 2; dc++) {
          ...
          if (nr >= 0 && nr < ROWS && nc >= 0 && nc < COLS && isJoker(currentGrid[nr][nc])) {
            const emoji = currentGrid[nr][nc] || JOKER;
            currentGrid[nr][nc] = null;
            spawnBouncingBead(nr, nc, emoji, i * 10 + dr * 5 + dc);
            hitAny = true;
          }
        }
      }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.download = `小丑嘉年華-感謝狀-${playerName || '表演者'}.jpg`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  function findMatchLines(g: (string | null)[][]) {
    ...
    for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
        const color = g[r][c];
        if (!color || isJoker(color)) continue;
  ...
  const spawnJokers = async (currentGrid: (string | null)[][], count: number) => {
    ...
    nextGrid.forEach((row, ri) => row.forEach((cell, ci) => {
      if (cell && !isJoker(cell)) candidates.push({ r: ri, c: ci });
    }));
    
    const newStats = { ...spawnStats };
    while (spawned < count && candidates.length > 0) {
      const idx = Math.floor(Math.random() * candidates.length);
      const pos = candidates.splice(idx, 1)[0];
      const emoji = JOKER_TYPES[Math.floor(Math.random() * JOKER_TYPES.length)];
      nextGrid[pos.r][pos.c] = emoji;
      performJokerFerrisWheel(pos.r, pos.c, emoji);
      spawned++;
      newStats[emoji] = (newStats[emoji] || 0) + 1;
    }
    setSpawnStats(newStats);
    
    if (nextGrid.flat().filter(isJoker).length >= STAMPEDE_THRESHOLD) {
  ...
  const handleCellClick = async (r: number, c: number) => {
      ...
      for (let dr = -2; dr <= 2; dr++) {
        for (let dc = -2; dc <= 2; dc++) {
          ...
          if (nr >= 0 && nr < ROWS && nc >= 0 && nc < COLS && nextGrid[nr][nc]) {
            ...
            if (isJoker(nextGrid[nr][nc])) {
              spawnBouncingBead(nr, nc, nextGrid[nr][nc]!); 
            }
            nextGrid[nr][nc] = null;
            hits++;
          }
    ...
    if (gameState === 'SPARKLING') {
      ...
      if (!color) return;
      shakeScreen();
      const nextGrid = grid.map(row => row.map(cell => (cell === color || isJoker(cell)) ? null : cell));
      grid.forEach((row, ri) => row.forEach((cell, ci) => {
        if (cell === color || isJoker(cell)) {
          setTimeout(() => { performJokerFerrisWheel(ri, ci, cell || JOKER); }, Math.random() * 300);
        }
      }));
    ...
    if (isJoker(grid[r][c])) { playPop(100, 'square'); return; }
  ...
  return (
    ...
    `}>
      ...
      {!isGameStarted && (
        <div className="fixed inset-0 z-[500] bg-indigo-950 flex flex-col items-center justify-center p-6 text-center">
          <div className="text-8xl mb-6 animate-bounce">🎪</div>
          <h1 className="text-4xl font-black text-white mb-2 italic tracking-tighter">小丑嘉年華</h1>
          <p className="text-indigo-300 text-sm mb-8 font-bold">CLOWN CARNIVAL: MAGIC GUIDE</p>
          
          <div className="w-full max-w-xs space-y-4">
            <div className="text-left">
              <label className="text-[10px] font-black text-indigo-400 uppercase tracking-widest ml-2">表演者暱稱</label>
              <input 
                type="text" 
                value={playerName}
                onChange={(e) => setPlayerName(e.target.value)}
                placeholder="輸入你的藝名..."
                className="w-full bg-indigo-900/50 border-2 border-indigo-700 rounded-2xl px-6 py-4 text-white font-bold placeholder:text-indigo-700 focus:outline-none focus:border-purple-500 transition-all"
              />
            </div>
            <button 
              onClick={() => setIsGameStarted(true)}
              className="w-full py-5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-black text-xl rounded-2xl shadow-xl hover:scale-105 active:scale-95 transition-all border-b-4 border-indigo-800"
            >
              開始表演 ➔
            </button>
          </div>
          
          <div className="mt-12 text-indigo-500 text-[10px] font-bold uppercase tracking-widest">
            國土永續2026年版 • 公益教育授權
          </div>
        </div>
      )}
      ...
      {gameState === 'SETTLEMENT' && (
        <div className="fixed inset-0 z-[200] bg-black/95 flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500 overflow-y-auto">
          <div ref={certificateRef} className="bg-indigo-950 p-6 rounded-[40px] border-4 border-double border-orange-500/50 w-full max-w-sm shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col items-center mb-6">
            <h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-b from-yellow-300 to-orange-600 mb-4 italic">
              表演感謝狀
            </h2>
            
            <div className="w-full border-y border-white/10 py-3 mb-4">
              <div className="text-neutral-500 text-[9px] font-black uppercase tracking-widest mb-1">傑出表演者</div>
              <div className="text-xl font-black text-white italic">{playerName || '無名英雄'}</div>
            </div>

            <div className="bg-black/40 backdrop-blur-md p-5 rounded-[30px] border border-orange-500/20 mb-4 w-full shadow-inner relative">
               <div className="text-neutral-500 text-[9px] font-black uppercase tracking-widest mb-1">演出最終得分</div>
               <div className="text-5xl font-black text-yellow-500">{score}</div>
            </div>
            
            <div className="bg-black/20 p-3 rounded-[24px] border border-white/5 mb-4 w-full text-center">
              <h4 className="text-white/80 font-black text-[9px] mb-2 italic uppercase tracking-widest">最佳表演搭檔 🎪</h4>
              {Object.entries(spawnStats).length > 0 ? (
                (() => {
                  const topPerformer = Object.entries(spawnStats).sort((a: [string, number], b: [string, number]) => b[1] - a[1])[0];
                  return (
                    <div className="flex flex-col items-center gap-1">
                      <span className="text-4xl animate-bounce">{topPerformer[0]}</span>
                      <span className="text-white/90 font-bold text-xs">演出 {topPerformer[1]} 次</span>
                    </div>
                  );
                })()
              ) : (
                <div className="text-white/40 text-xs italic">無演出紀錄</div>
              )}
              <div className="mt-2 text-[8px] text-neutral-600 font-bold uppercase">
                總計參與演出：{Object.values(spawnStats).reduce((a: number, b: number) => a + b, 0)} 位表演者
              </div>
            </div>

            {/* 版權與公益資訊標籤 */}
            <div className="p-3 bg-white/5 rounded-2xl border border-white/10 w-full text-center">
              <div className="text-slate-500 text-[9px] font-bold mb-1">國土永續2026年版</div>
              <div className="text-slate-600 text-[7px] leading-tight">使用者公益宣導與教育使用，授權不得商業轉售。</div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 w-full max-w-xs">
            <button 
              ...
              className={`px-12 py-4 bg-white text-indigo-950 font-black text-xl rounded-full transition-all border-b-4 border-indigo-200 shadow-lg flex items-center justify-center gap-2 ${isExporting ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105 active:scale-95'}`}
            >
              {isExporting ? '正在產生...' : '💾 儲存感謝狀 (JPG)'}
            </button>
            <button onClick={initGrid} className="px-12 py-4 bg-gradient-to-r from-orange-500 to-pink-500 text-white font-black text-xl rounded-full hover:scale-105 active:scale-95 transition-all border-b-4 border-orange-800 shadow-lg">
              重新開始表演 🔄
            </button>
          </div>
        </div>
